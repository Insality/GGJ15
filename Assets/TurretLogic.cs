...
public class TurretLogic: MonoBehaviour {
    ...
    private void PatternLaserAllSide(int i)
    {
        if (TurretNumber == i && _curBeat % FireBeatTemp == 0)
        {
            ...
            _proj = Instantiate(LaserProjectile) as GameObject;
            _proj.transform.position = transform.position + HexagonUtils.GetV3FromV2(HexagonUtils.GetVectorBySide((TurretNumber + 4) % 6));
            _proj.GetComponent<WarningLogic>().Direction = ((TurretNumber + 4) % 6);

            _proj = Instantiate(LaserProjectile) as GameObject;
            _proj.transform.position = transform.position + HexagonUtils.GetV3FromV2(HexagonUtils.GetVectorBySide((TurretNumber + 2) % 6));
            _proj.GetComponent<WarningLogic>().Direction = ((TurretNumber + 2) % 6);
